...
export default function Hero() {
  ...
  return (
    <section
      id="home"
      className="relative min-h-[70vh] pt-28 pb-16 flex flex-col justify-center items-center px-4 sm:px-6 lg:px-8 overflow-hidden"
    >
      ...
      <div className="max-w-4xl w-full text-center relative z-10 flex flex-col items-center">
        ...
        >
          <button
            ...
            className="px-8 py-3.5 rounded-full bg-[var(--color-primary)] text-[var(--color-cream)] font-bold text-sm hover:bg-[var(--color-dark-coffee)] hover:scale-105 transition-all shadow-md flex items-center gap-2.5 cursor-pointer"
          >
            <Sparkles className="w-4 h-4 text-amber-300" />
            <span>{isExpanded ? 'Hide Hero Information ▲' : 'View Hero Information ✨'}</span>
          </button>
        </motion.div>

        {/* Collapsible Hero Information */}
        <AnimatePresence>
          {isExpanded && (
            <motion.div
            ...
            >
              {/* Welcome Note Card */}
              <div className="journal-paper p-6 sm:p-8 max-w-2xl w-full mb-8 text-left shadow-sm">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 mb-8">
                <a
                  href="#journey"
                  onClick={() => window.dispatchEvent(new CustomEvent('nav-section-click', { detail: '#journey' }))}
                ...
                <a
                  href="#contact"
                  onClick={() => window.dispatchEvent(new CustomEvent('nav-section-click', { detail: '#contact' }))}
                  className="px-8 py-3.5 rounded-full border border-[var(--border-accent)] bg-[var(--bg-card)] text-[var(--text-primary)] font-medium hover:border-[var(--color-primary)] hover:scale-105 transition-all flex items-center gap-2 text-decoration-none"
                >
                  <span>Say Hello</span>
              ...
              <a
                href="#about"
                onClick={() => window.dispatchEvent(new CustomEvent('nav-section-click', { detail: '#about' }))}
                className="mt-4 text-xs text-[var(--text-muted)] flex flex-col items-center gap-1 group text-decoration-none mb-4"
              >
